...
```{r testresults}
x <- table(altg = sim1$genes,sign=tt$p.value<0.05)
x
prop.table(x,2)
```

a. How many false-positives are there (unadj p<0.05)?
...
x <- table(altg = sim1$genes,sign=Bonf.adjp<0.05)
x
prop.table(x,2)
```
...
x <- table(altg = sim1$genes,sign=FDR.adjp<0.05)
x
prop.table(x,2)
```
...
### a. Use Bonferroni adjusted p<0.05
...
x <- table(altg = (sim1$genes)[fidx],sign=Bonf.adjp<0.05)
x
prop.table(x,2)
```

### b. Use Benjamini-Hochberg FDR adjusted p<0.05
...
x <- table(altg = (sim1$genes)[fidx],sign=BH.adjp<0.05)
x
prop.table(x,2)
```
