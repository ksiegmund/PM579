---
title: "Lab7. Gene Networks-mouse liver"
author: "ks"
date: "7/12/2017"
...
if(!require('preprocessCore'))   biocLite("preprocessCore")
if(!require('GO.db')) biocLite("GO.db")
#install.packages("WGCNA")

if(!require('cluster')) biocLite('cluster')
if(!require(Hmisc)) biocLite('Hmisc')

#install.packages(c("dynamicTreeCut", "cluster", "Hmisc", "reshape", "foreach", "doParallel") )
...
```{r readData}
dataDir = c("~kims/Google Drive File Stream/My Drive/PM579/Data/WGCNA/FemaleLiver-Data");
...
### 1b. Check for outlier samples based on # of missing values.
...
Read the help files for how to modify defaults for different data sets. Read the .pdf on the website for words of caution with data sets of size >5000 features.
...
Constructing a weighted gene network entails the choice of the soft thresholding power β to which co-expression
similarity is raised to calculate adjacency [1].
Earlier we saw that 6 was a good power for these data using the criterion of approximate scale-free topology. 
...
One way to visualize a weighted network is to plot its heatmap. Each row and column of the heatmap correspond to a single gene. The heatmap can depict adjacencies or topological overlaps, with light colors denoting
...
such network plots. This TOM plot too big for quick viewing, try on smaller number of genes.
...
The function produces a dendrogram of the eigengenes and trait(s), and a heatmap of their relationships. To split
the dendrogram and heatmap plots, we can use the following code
```{r plotseparately}
# Plot the dendrogram
#sizeGrWindow(6,6);
par(cex = 1.0)
