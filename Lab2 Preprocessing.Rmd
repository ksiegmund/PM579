...
head(unique(rawdata$genes$Status),n=10)
```
```{r table cpp}
rawdata$genes$Status[which(substr(rawdata$genes$Status,1,4)=="ERCC")]="ERCC"
...
```{r sva}
design=model.matrix(~factor(jbc$targets$Type))
design0= model.matrix(~1,data=as.data.frame(jbc$targets$Type))
...
We can also try 1 variable and see how that goes.
```{r sva1var}
design=model.matrix(~factor(jbc$targets$Type))
design0= model.matrix(~1,data=as.data.frame(jbc$targets$Type))
svobj = sva(jbc$E,design,design0,n.sv=1)
modsv=model.matrix(~svobj$sv)
fit=lmFit(jbc$E,modsv)
yhat=fit$coef %*% t(modsv)
jbc.svresid=jbc$E-yhat
