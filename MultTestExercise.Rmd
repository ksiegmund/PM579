...
Illustrate the concept of power by randomly sampling from the population 1,000 times and testing for differential expression.  Let's create a [100 x 1000] matrix where each column is a simulated data set.
```{r 1gene2gp}
...
# Get 7500 gene expression values for single observation
# Repeat for 100 individuals(Gp1 + Gp2)
nullmat <- replicate(100,rnorm(nullgenes, mean = 0, sd =
                                  sim_sd[1:nullgenes]))
# Get 2500 gene expression values for Gp1 individual (mean = 0)
# Repeat for 50 Gp1 individuals
signmatg1 <- replicate(50,rnorm(signgenes, mean = 0, sd = 
                                  sim_sd[(nullgenes+1):ng]))
# Get 2500 gene expression values for Gp2 individuals (mean = 0.7)
# Repeat for 50 Gp2 individuals
signmatg2 <- replicate(50,rnorm(signgenes, mean = 0.7, sd = 
                                  sim_sd[(nullgenes+1):ng]))
# Columns are individuals (Gp1,Gp2)
signmat <- cbind(signmatg1,signmatg2)

# Rows have 7500 Null genes followed by 2500 DE genes
