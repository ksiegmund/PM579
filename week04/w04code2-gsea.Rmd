...
The sessionInfo() later will tell us this is version 7.5.1, which we can see is the same version as posted to the website. Last summer, the R version was older than the updated website.
...
There are a total of `r length(hs.c2.gsnames)` curated gene sets. ~1000 curated gene sets have been added in the last year.
...
The median gene set size is `r median(map_int(hs.c2.msList,length))`, and the smallest is only `r min(map_int(hs.c2.msList,length))`.
...
```{r gs}
    ng <- length(unlist(hs.c2.msList["NELSON_RESPONSE_TO_ANDROGEN_UP"]))
  ng
```
...
This gene set has `r ng` genes, but there are $>$ 1.5 features per gene on average. 
...
This represents `r round(length(rt)/length(hs.c2.gsnames)*100,0)`\% of the gene sets in our list.
